...
impl TextEdit<'_> {
    ...
    fn show_content(self, ui: &mut Ui) -> TextEditOutput {
        ...
        let mut cursor_range = state.cursor.range(&galley);
        ...
        if clip_text && align_offset.x == 0.0 {
            let cursor_pos = match cursor_range {
                Some(cursor_range) => galley.pos_from_cursor(cursor_range.primary).min.x,
                _ => 0.0,
            };
        ...
        if ui.is_rect_visible(rect) {
            ...
            if let Some(cursor_range) = state.cursor.range(&galley) {
                ...
                // Always paint selection, even without focus, so the user can
                // see their selection while interacting with other widgets.
            ...
            if let Some(cursor_range) = state.cursor.range(&galley) {
                ...
                if text.is_mutable() && interactive {
                    ...
                    // Only show (and blink) cursor if the egui viewport has focus, OR
                    // if this widget still has focus. Widget focus is needed for the blinking
                    // cursor to indicate the text can be typed into.
                    let viewport_has_focus = ui.ctx().input(|i| i.focused);
                    if viewport_has_focus && has_focus {
                    ...
                    // Only when widget has focus, since IME is harmful when the user is just
                    // viewing a selection
                    if has_focus {
                        let to_global = ui
                            .ctx()
