    ...
    for placed_row in &mut rows {
        ...
        for glyph in &mut row.glyphs {
            ...
            glyph.pos.y = glyph.font_impl_ascent
                ...
                // Use `font_height` instead of `line_height` so that an explicit `TextFormat::line_height`
                // (used to add line spacing) does not zero out the valign term — otherwise the glyphs
                // stick to the top of the row and all extra spacing accumulates at the bottom.
